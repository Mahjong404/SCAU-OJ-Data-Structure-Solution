...
int main() {
    int length=0,x=1,count=0;   //length为hash表容量，x为输入值，count为实际插入元素数量
    double fuck=0;              //fuck为累计冲突次数
    vector<int> hash(1001);     //必须初始化
    cin>>length;
    cin>>x;
    while(x!=-1){
        int pos=(3*x)%length;   //hash地址算法
        while(hash[pos]){       //当前位置非0
            fuck++;             //冲突+1
            pos++;              //后移1位
        }
        hash[pos]=x;            //插入操作
    ...
    cout<<endl;
    cout<<"Average search length="<<fixed<<setprecision(6)<<fuck/count+1;
    return 0;
}
